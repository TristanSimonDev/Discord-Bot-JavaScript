...
const EventFiles = fs
    .readdirSync("src/Bots/Bot1/Events/")
    .filter((file) => file.endsWith(".js"));
...
for (const file of EventFiles) {
    ...
    console.log(
        ColorOutput(
            `Event [${Event.Name}]-[ID: ${Event.EventID}] successfully loaded Dynamically`
        ).green
    );

    try {
        client[Event.Type](Event.Name, async (...args) =>
            Event.execute(...args)
        );
    } catch (err) {
        console.log(ColorOutput(`Error: ${err}`).red);
    }
}
...
process.on("SIGINT", () => {
    console.log(
        ColorOutput("Received SIGINT. Gracefully shutting down...").green
    );
...
// Login to Discord
client
    .login(process.env.TokenForChatBot)
    .then(() => {
        console.log(
            ColorOutput(
                `\n=== Bot Login Info ===` +
